...
__all__ = [
    "BumpsOptions",
    "interpret_fit_options",
    "reload_export",
    "main",
    # CRUFT: symbols from old cli interface still used by third parties
    "load_best",
    "load_model",
    "save_best",
    "config_matplotlib",
...
from .fitproblem import load_problem as load_model
from .fitters import save_best  # used by scattertools
...
def main(options: Optional[BumpsOptions] = None):
    # Redirect to old interface if --wx is added to the command line
    # You can also invoke it as python -m bumps.gui
    if options is None and "--wx" in sys.argv:
        from bumps.gui.old_cli import main as old_main

        sys.argv = [s for s in sys.argv if s != "--wx"]
        return old_main()

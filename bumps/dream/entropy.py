...
file,\ [#Turjillo-Ortiz]_\ [#Mardia1970]_\ [#Mardia1974]_\ [#Stevens]_ as
...
def entropy(points, logp, N_entropy=10000, N_norm=2500):
    ...
    *N_entropy* is the number of points used to estimate the entropy
    ...
    if False:
        ...
        idx = plt.argsort(entropy_points[:, 0])
        plt.figure()
        plt.subplot(221)
        plt.hist(points[:, 0], bins=50, density=True, log=True)
        plt.plot(entropy_points[idx, 0], rho[idx], label="density")
        plt.plot(entropy_points[idx, 0], exp(eval_logp + log_scale)[idx], label="p")
        plt.ylabel("p(x)")
        plt.legend()
        plt.subplot(222)
        plt.hist(points[:, 0], bins=50, density=True, log=False)
        plt.plot(entropy_points[idx, 0], rho[idx], label="density")
        plt.plot(entropy_points[idx, 0], exp(eval_logp + log_scale)[idx], label="p")
        plt.ylabel("p(x)")
        plt.legend()
        plt.subplot(212)
        plt.plot(entropy_points[idx, 0], frac[idx], ".")
        plt.xlabel("P[0] value")
        plt.ylabel("p(x)/kernel density")

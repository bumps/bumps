...
class Connection(object):
    ...
    def post(self, resource, fields={}, body=None, mimetype=DEFAULT_CONTENT):
        return _request(self.http, "POST", self.url + resource, fields=fields, body=body, mimetype=mimetype)

    def put(self, resource, fields={}, body=None, mimetype=DEFAULT_CONTENT):
        return _request(self.http, "PUT", self.url + resource, fields=fields, body=body, mimetype=mimetype)

    def postfiles(self, resource, fields={}, files=None):
        return _request(self.http, "POST", self.url + resource, fields=fields, files=files)

    def putfiles(self, resource, fields={}, files=None):
        return _request(self.http, "PUT", self.url + resource, fields=fields, files=files)

    def delete(self, resource, fields={}):
        return _request(self.http, "DELETE", self.url + resource, fields=fields)


def _request(http, verb, location, fields=None, body=None, mimetype=None, files=None):
    ...
    if files:
        ...
        if verb == "GET":
            location += "?" + parse.urlencode(fields)
            body = ""
        else:
    ...
    # print "method",verb
    try:
        response, content = http.request(location, verb, body=body, headers=headers)
